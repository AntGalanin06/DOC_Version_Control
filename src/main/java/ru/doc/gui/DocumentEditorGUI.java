...
public class DocumentEditorGUI extends JFrame implements DocumentObserver {
    ...
    private JRadioButton debugBtn;
    private JRadioButton errorBtn;
    private JRadioButton allCatBtn;
    private JRadioButton docCatBtn;
    private JRadioButton histCatBtn;
    private JRadioButton factCatBtn;
    private JRadioButton decCatBtn;
    ...
    public void initialize() {
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                logCollector.add(LogCollector.Level.ERROR, LogCollector.Category.UI, LogCollector.OperationType.ERROR, "Initialization error: " + e.getMessage());
    ...
    private void init() {
        String[] names = factories.stream().map(DocumentFactory::getName).toArray(String[]::new);
        String kind = (String) JOptionPane.showInputDialog(null, "Document type", "New document",
                JOptionPane.QUESTION_MESSAGE, null, names, names[0]);
        if (kind == null) System.exit(0);
        DocumentFactory factory = factories.stream().filter(f -> f.getName().equals(kind)).findFirst().orElseThrow();
        ...
        opts.add(encCB);
        if (JOptionPane.showConfirmDialog(null, opts, "Decorators", JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION)
            System.exit(0);
        if (encCB.isSelected()) {
    ...
    private void buildUI(String kind) {
        ...
        historyList.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
                    int idx = historyList.locationToIndex(e.getPoint());
                    if (idx >= 0) {
                        context.getBean(RevertCommand.class, document, logger, idx).execute();
                        logger.addMemento(document.createMemento());
                        logger.clearRedo();
                        refresh();
                    }
        ...
        undoBtn.addActionListener(e -> {
            context.getBean(UndoCommand.class, document, logger).execute();
            refresh();
        });
        redoBtn.addActionListener(e -> {
            context.getBean(RedoCommand.class, document, logger).execute();
            refresh();
        });
        ...
        filterPanel.add(infoBtn);
        filterPanel.add(debugBtn);
        filterPanel.add(errorBtn);
        ...
        docCatBtn = new JRadioButton("DOCUMENT");
        histCatBtn = new JRadioButton("HISTORY");
        factCatBtn = new JRadioButton("FACTORY");
        decCatBtn = new JRadioButton("DECORATOR");
        categoryGroup.add(allCatBtn);
        categoryGroup.add(docCatBtn);
        categoryGroup.add(histCatBtn);
        categoryGroup.add(factCatBtn);
        categoryGroup.add(decCatBtn);
        categoryPanel.add(new JLabel("Filter category:"));
        categoryPanel.add(allCatBtn);
        categoryPanel.add(docCatBtn);
        categoryPanel.add(histCatBtn);
        categoryPanel.add(factCatBtn);
        categoryPanel.add(decCatBtn);
        allCatBtn.addActionListener(e -> refreshLogs());
        docCatBtn.addActionListener(e -> refreshLogs());
        histCatBtn.addActionListener(e -> refreshLogs());
    ...
    private void editContent() {
        ...
        if (newText != null && !newText.equals(current)) {
            context.getBean(ChangeTextCommand.class, document, newText).execute();
            logger.addMemento(document.createMemento());
            logger.clearRedo();
            refresh();
        }
    ...
    private void jumpToHistory(int index) {
        ...
        if (target != null) {
            context.getBean(RevertCommand.class, document, logger, index).execute();
        ...
        if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                if (!logger.getHistory().isEmpty()) {
                    DocumentMemento last = logger.getHistory().get(logger.getHistory().size() - 1);
    ...
    private void refresh() {
        historyModel.clear();
        IntStream.range(0, logger.getHistory().size()).forEach(i -> historyModel.addElement("#" + i));
    ...
    private void refreshLogs() {
        ...
        if (allBtn.isSelected()) {
            if (allCatBtn.isSelected()) {
                logs = logCollector.getRecords();
            } else if (docCatBtn.isSelected()) {
                logs = logCollector.getRecordsByCategory(LogCollector.Category.DOCUMENT);
            } else if (histCatBtn.isSelected()) {
                logs = logCollector.getRecordsByCategory(LogCollector.Category.HISTORY);
            } else if (factCatBtn.isSelected()) {
                logs = logCollector.getRecordsByCategory(LogCollector.Category.FACTORY);
            } else {
                logs = logCollector.getRecordsByCategory(LogCollector.Category.DECORATOR);
            }
        } else {
            LogCollector.Level level = infoBtn.isSelected() ? LogCollector.Level.INFO :
                    debugBtn.isSelected() ? LogCollector.Level.DEBUG :
                            LogCollector.Level.ERROR;
            if (allCatBtn.isSelected()) {
                logs = logCollector.getRecordsByLevel(level);
            } else {
                LogCollector.Category category = docCatBtn.isSelected() ? LogCollector.Category.DOCUMENT :
                        histCatBtn.isSelected() ? LogCollector.Category.HISTORY :
                                factCatBtn.isSelected() ? LogCollector.Category.FACTORY :
                                        LogCollector.Category.DECORATOR;
                logs = logCollector.getRecordsByCategoryAndLevel(category, level);
            }
        }
